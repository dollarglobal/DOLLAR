...
// Copyright (c) 2013 The DollarCoin Developers
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "DollarCoin server has now stopped running!";
}
...
// DollarCoin: Return average network hashes per second based on last number of blocks.
...
Value setaccount(const Array& params, bool fHelp)
{
    ...
    if (!address.IsValid())
        throw JSONRPCError(-5, "Invalid DollarCoin address");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(-5, string("Invalid DollarCoin address:")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; DollarCoin server stopping, restart to run with encrypted wallet";
}
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "DollarCoin is not connected!!!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "DollarCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (params.size() == 0)
    {
        ...
        result.push_back(Pair("target",   HexStr(BEGIN(hashTarget), END(hashTarget))));
        result.push_back(Pair("algorithm", "scrypt:1024,1,1"));  // DollarCoin: specify that we should use the scrypt algorithm
        return result;
    }
    else
    {
...
void ThreadRPCServer2(void* parg)
{
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 11888));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "11888")))
